...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		// Default route after logining successfully
		$defaultRoute = 'catalog/product';

		$this->document->setTitle($this->language->get('heading_title'));

		if ($this->user->isLogged() && isset($this->request->get['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			$this->redirect($this->url->link($defaultRoute, 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($this->request->post['redirect'])) {
				...
				// Maybe unnecessary, cause the redirect url has been setted in login page
				$this->redirect($this->url->link($defaultRoute, 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
