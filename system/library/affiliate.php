	...
	private $code;
  	...
  	public function __construct($registry) {
		...
		if (isset($this->session->data['affiliate_id'])) {
			$affiliate_query = $this->db->get_where('affiliate', array('affiliate_id' => (int)$this->session->data['affiliate_id'], 'status' => 1));
			...
			if ($affiliate_query->num_rows) {
				...
				$this->code = $affiliate_query->row['code'];
      			...
      			$this->db->update('affiliate', array('ip' => $this->request->server['REMOTE_ADDR']), array('affiliate_id' => (int)$this->session->data['affiliate_id']));
			} else {
				$this->logout();
			}
  		}
  	...
  	public function login($email, $password) {
		$affiliate_query = $this->db->from('affiliate')
			->where(array('email' => $email, 'status' => 1, 'approved' => 1))
